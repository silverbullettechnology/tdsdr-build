...
#define MAX_ARGS    32
#define SEP_CHARS   " \t\n\r"
#define DEF_DEV_NUM 3
#define EXPORT_FILE "/tmp/.cal_ctl.channels"
...
static void export_active_channels (void)
{
	...
	{
		if ( (reg = cal_ctl_hal_spi_reg_get(adr)) < 0 )
			reg = 0;
		*fp++ = reg & 0xC0;
	}
...
/*	for ( i = 0; i < 3; i++ )
	...
	// TODO: UART
	if ( cal_ctl_hal_linux_spi_init(opt_dev_node) )
	{
		fprintf(stderr, "cal_ctl_hal_linux_spi_init(%s): %s\n",
		        opt_dev_node, strerror(errno));
		return -1;
	}

/*	if ( cal_ctl_hal_linux_gpio_init(opt_dev_gpio) )
	{
		fprintf(stderr, "cal_ctl_hal_linux_gpio_init(%d,%d,%d): %s\n",
...
void usage (void)
{
	printf("Run command: cal_ctl [options] command [args...]\n"
	       "Run script : cal_ctl [options] script\n"
	       "Interactive: cal_ctl [options]\n\n"
	       "Options: cal_ctl [-ei] [-d num] [-D node] [-l dir]\n"
	       "Where:\n"
...
/*	       "-g gpio GPIO pins for TXNRX,ENABLE,RESETN (default %d,%d,%d)\n"		*/
	       "-d num  Specify CAL_CTL device number (0-1, default %d)\n"
	       "-D node Specify CAL_CTL device node (default %s)\n"
...
int command (int argc, char **argv)
{
	...
	if ( !(map = cal_ctl_map_find(argv[0])) )
	{
		char  buff[PATH_MAX];
		char *path = argv[0];
	...
	if ( argc < map->argc )
	{
		...
		cal_ctl_map_help(map->name);
		return opt_soft_fail ? 0 : -1;
	}

	// errors in the actual library/middleware layers are >0
	ret = cal_ctl_map_call(map, argc, (const char **)argv);
...
int interact (FILE *fp)
{
	...
	rl_attempted_completion_function = cal_ctl_map_completion;
...
/*			case 'g':
				...
				break;
*/
			case 'd': // Specify CAL_CTL device number
				errno = 0;
	...
	char *p;
	if ( ! opt_lib_dir )
		opt_lib_dir = (p = getenv("CAL_CTL_LIB_DIR")) ? p : LIB_DIR;

	if ( (p = getenv("CAL_CTL_SCRIPT_PATH")) )
	...
	cal_test_function();
	cal_ctl_dev_reopen(0,0);
	UINT16 cal_data[] = {1,2,3,4,5};

	cal_ctl_spiwritearray(&cal_data[0], 5);
