...
LOG_MODULE_STATIC("seq_open", LOG_LEVEL_DEBUG);


static struct v49_common req_open = 
{
	...
	.command = 
	{
		...
		.request   = V49_CMD_REQ_OPEN,
	}
};


static int exp_open (struct mbuf *mbuf, void *data)
{
	...
	if ( (ret = expect_common(&rsp, mbuf, &req_open, data)) < 1 )
		RETURN_VALUE("%d", ret);
...
int seq_open (struct socket *sock, uint32_t stream_id)
{
	...
	req_open.sid = stream_id;
	LOG_DEBUG("Open: SID %u\n", req_open.sid);
	...
	if ( (ret = v49_format(&req_open, mbuf, NULL)) != V49_OK_COMPLETE )
	{
		LOG_ERROR("v49_format(open) failed: %s\n", v49_return_desc(ret));
		mbuf_deref(mbuf);
	}
	expect_send(sock, mbuf);

	ret = expect_loop(sock, map_open, s_to_clocks(5));
