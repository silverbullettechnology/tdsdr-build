...
LOG_MODULE_STATIC("seq_release", LOG_LEVEL_DEBUG);


static struct v49_common req_release = 
{
...
static int exp_release (struct mbuf *mbuf, void *data)
{
	...
	if ( (ret = expect_common(&rsp, mbuf, &req_release, data)) < 1 )
		RETURN_VALUE("%d", ret);

	LOG_DEBUG("RELEASE success\n");
	RETURN_ERRNO_VALUE(0, "%d", 1);
}

struct expect_map map_release[] = 
{
	{ exp_release, NULL },
	{ NULL }
};
...
int seq_release (struct socket *sock, uuid_t *cid, uint32_t stream_id)
{
	...
	if ( (ret = v49_format(&req_release, mbuf, NULL)) != V49_OK_COMPLETE )
	{
		LOG_ERROR("v49_format(release) failed: %s\n", v49_return_desc(ret));
		mbuf_deref(mbuf);
	}
	expect_send(sock, mbuf);

	ret = expect_loop(sock, map_release, s_to_clocks(5));
